...
const GetQuotePhone: FC = () => {
  const dispatch = useAppDispatch();
  const { manufacturerId, modelId } = useAppSelector((state) => state.item.ids);
  const { device, manufacturer, model, storage, color, carrier } =
    useAppSelector((state) => state.item);
  const [disable, setDisable] = useState<boolean>(true);

  useEffect(() => {
    if (device === Phone || device === Phones) {
      manufacturer && model && storage && setDisable(false);
    }
    if (device === Tablet || device === Tablets) {
      manufacturer && model && storage && carrier && setDisable(false);
    }
  }, [manufacturer, model, storage, carrier, color]);
...
export default GetQuotePhone;
function handler(arg0: string, handler: any, arg2: { passive: true }) {
